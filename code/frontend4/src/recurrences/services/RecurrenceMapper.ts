...
export class RecurrenceMapper {
    ...
    mapToDto(items: PlannedRecurrenceModel[]): PlannedRecurrenceDto[] {
        return items.map(x => ({
            uid: x.uid,
            task: x.task,
            startDate: this.dateService.changeFromLocalToUtc(
                new Date(x.startDate),
            ),
            endDate: x.endDate
                ? this.dateService.changeFromLocalToUtc(new Date(x.endDate))
                : null,
    ...
    mapToModel(items: PlannedRecurrenceDto[]): PlannedRecurrenceModel[] {
        return items.map(x => ({
            ...
            startDate: this.dateService
                .changeFromUtcToLocal(x.startDate)
                .valueOf(),
            endDate: x.endDate
                ? this.dateService.changeFromUtcToLocal(x.endDate).valueOf()
                : null,
            everyNthDay: x.everyNthDay,
            everyMonthDay: x.everyMonthDay,
            everyWeekday: x.everyWeekday,
