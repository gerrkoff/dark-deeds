...
test('renders no time', () => {
    const model = new Task('1', 'Title', new Date(2018, 1, 1), 1, false, false, false, TaskTypeEnum.Simple)
...
test('renders concrete time', () => {
    const model = new Task('1', 'Title', new Date(2018, 1, 1), 1, false, false, false, TaskTypeEnum.Simple, false, 0, 61)
    const component = enzyme.shallow(enzyme.shallow(<TaskItem task={model} />).prop('content'))
    expect(component.find('span').text()).toBe('01:01 Title')
})

test('renders completed', () => {
    const model = new Task('1', 'Title', new Date(2018, 1, 1), 1, false, true, false, TaskTypeEnum.Simple, false, 0, 61)
    const component = enzyme.shallow(enzyme.shallow(<TaskItem task={model} />).prop('content'))
    expect(component.find('span.task-item-completed').length).toBe(1)
})

test('renders probable', () => {
    const model = new Task('1', 'Title', null, 1, false, false, false, TaskTypeEnum.Simple, true)
    const component = enzyme.shallow(enzyme.shallow(<TaskItem task={model} />).prop('content'))
    expect(component.find('span.task-item-probable').length).toBe(1)
})

test('renders additional', () => {
    const model = new Task('1', 'Title', new Date(2018, 1, 1), 1, false, false, false, TaskTypeEnum.Additional)
    const component = enzyme.shallow(enzyme.shallow(<TaskItem task={model} />).prop('content'))
    expect(component.find('span').text()).toBe('Title')
})
