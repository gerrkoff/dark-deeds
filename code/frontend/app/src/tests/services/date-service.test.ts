...
test('[toDateFromSpecialFormat] positive', () => {
    ...
    expect(result!.getTime()).toBe(new Date(2018, 6, 4).getTime())
})

test('[toDateFromSpecialFormat] null if incorrect string', () => {
    const service = new DateService()
    expect(service.toDateFromSpecialFormat('123456789')).toBe(null)

    expect(service.toDateFromSpecialFormat('1234567')).toBe(null)

    expect(service.toDateFromSpecialFormat('1234!678')).toBe(null)

    expect(service.toDateFromSpecialFormat('1234a678')).toBe(null)
...
test('[toLabel] positive', () => {
    const service = new DateService()
    expect(service.toLabel(new Date(2018, 9, 21))).toBe('10/21/2018 Sun')
})

test('[monday] positive', () => {
    const service = new DateService()
    expect(service.monday(new Date(2018, 9, 17)).getTime()).toBe(
        new Date(2018, 9, 15).getTime()
    )
})
...
test('[adjustDatesAfterLoading] positive', () => {
    const arr: any = [
        new Task('1', ''),
        { uid: '2', date: '2018-11-14T18:00:00Z' },
    ]
