...
export function createRecurrences() {
    return async (dispatch: ThunkDispatch<actions.RecurrencesViewAction>) => {
        dispatch({
            type: actions.RECURRENCESVIEW_CREATING_RECURRENCES_PROCESSING,
        })

        try {
            const createdRecurrencesCount =
                await recurrencesApi.createRecurrences(
                    dateService.getTimezoneOffset()
                )
            toastService.success(
                `${createdRecurrencesCount} recurrences were created`
            )
        } catch (err) {
...
export function loadRecurrences() {
    return async (dispatch: ThunkDispatch<actions.RecurrencesViewAction>) => {
        dispatch({
            type: actions.RECURRENCESVIEW_LOADING_RECURRENCES_PROCESSING,
        })

        try {
            const plannedRecurrences = await recurrencesApi.loadRecurrences()
            dispatch({
                type: actions.RECURRENCESVIEW_LOADING_RECURRENCES_SUCCESS,
                plannedRecurrences,
            })
        } catch (err) {
...
export function saveRecurrences(recurrences: PlannedRecurrence[]) {
    return async (dispatch: ThunkDispatch<actions.RecurrencesViewAction>) => {
        ...
        try {
            const updatedRecurrencesCount =
                await recurrencesApi.saveRecurrences(recurrences)
            toastService.success(
                `${updatedRecurrencesCount} recurrences were updated`
            )
            dispatch({
                type: actions.RECURRENCESVIEW_CHANGE_EDITTING_RECURRENCE,
