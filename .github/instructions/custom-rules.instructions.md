...
**Use .NET SDK `8.0.302` as the minimum across CI and Docker build surfaces, while `code/backend/global.json` uses `latestFeature` so local development can roll forward to a newer installed .NET 8 feature band.** Run terminal release and CI `dotnet` commands from `code/backend` so the nested `global.json` governs SDK resolution; `setup-dotnet` installing an SDK does not force selection when `global.json` is outside the command's working-directory ancestry.
